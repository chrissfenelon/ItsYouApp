...
import { Puissance4Service, Puissance4Game } from '../../../services/Puissance4Service';
import { AvatarDisplay } from '../../../utils/avatarUtils';
import CustomAlert from '../../../components/common/CustomAlert';
import { useCustomAlert } from '../../../hooks/useCustomAlert';
import Puissance4StakesModal from '../../../components/games/puissance4/Puissance4StakesModal';
import { Puissance4Stakes } from '../../../types/puissance4.types';

const { width } = Dimensions.get('window');

const Puissance4LobbyScreen: React.FC<any> = ({ route, navigation }) => {
  const { gameId } = route.params || {};
  const { user } = useApp();
  const { alertConfig, isVisible, showAlert, hideAlert } = useCustomAlert();

  const [game, setGame] = useState<Puissance4Game | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = Puissance4Service.subscribeToGame(
      gameId,
      (updatedGame) => {
        setGame(updatedGame);
        setIsLoading(false);
        ...
        if (updatedGame.status === 'playing') {
          ...
          const currentPlayerData = updatedGame.players.find(p => p.id === user.id);
          const playerColor = currentPlayerData?.color || 'Rouge';

          navigation.replace('puissance4Game', {
            mode: 'online',
            gameId,
            playerId: user.id,
            playerColor,
            stakes: updatedGame.stakes || undefined, // Use undefined instead of null
  ...
  const handleToggleReady = async () => {
    ...
    try {
      ...
      await Puissance4Service.setPlayerReady(gameId, user.id, newReadyState);
  ...
  const handleStartGame = async () => {
    ...
    try {
      await Puissance4Service.startGame(gameId, user.id);
      FeedbackService.success();
    } catch (error: any) {
      ...
      buttons: [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              if (user?.id) {
                await Puissance4Service.leaveGame(gameId, user.id);
              }
              navigation.goBack();
            } catch (error) {
  ...
  const handleSetStakes = () => {
    FeedbackService.buttonPress();
    setShowStakesModal(true);
  };

  const handleStakesConfirm = async (stakes: Puissance4Stakes) => {
    if (!game || !user?.id) return;

    try {
      await Puissance4Service.updateGameStakes(gameId, stakes);
      FeedbackService.success();
      ...
      showAlert({
        title: 'Erreur',
        message: 'Impossible de définir les enjeux',
        type: 'error',
      });
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <View style={styles.blurryOverlay}>
          ...
          >
            ...
            {/* Stakes Section */}
            {game.stakes && game.stakes.type !== 'none' && game.stakes.description ? (
              <View style={styles.stakesCard}>
                <View style={styles.stakesHeader}>
                  <Text style={styles.sectionTitle}>💝 Enjeu de la partie</Text>
                  {isHost && (
                    <TouchableOpacity onPress={handleSetStakes}>
                      <MaterialCommunityIcons
                        name="pencil"
                        size={20}
                        color={CurrentTheme.romantic.primary}
                ...
                <View style={styles.stakeRow}>
                  ...
                  <Text style={styles.stakeText}>
                    Le perdant devra : {game.stakes.description}
                  </Text>
                </View>
              </View>
            ) : (
              isHost && (
                <TouchableOpacity
                  style={styles.addStakesButton}
                  onPress={handleSetStakes}
                >
                  <MaterialCommunityIcons name="plus-circle" size={24} color={CurrentTheme.romantic.primary} />
                  <Text style={styles.addStakesText}>Ajouter un enjeu (optionnel)</Text>
                </TouchableOpacity>
              )
            )}
      ...
      <Puissance4StakesModal
        visible={showStakesModal}
        onClose={() => setShowStakesModal(false)}
        onConfirm={handleStakesConfirm}
        mode="online"
      />
    </View>
  );
...
export default Puissance4LobbyScreen;
